...
public class RadialSlider: MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler
{
	...
	public void Update() 
	{
		//Просто поделить время в секундах на 10
		if (myTimerManager.isTimerOn) 
		{
			double angleAdjust = myTimerManager.timeToDisplay / 3600f;
			GetComponent<Image>().fillAmount = ((float)angleAdjust);
		}
	...
	IEnumerator TrackPointer()
	{
		...
		if( ray != null && input != null )
		{
			...
			{                    
				...
				if (isPointerDown && !myTimerManager.isTimerOn && !myTimerManager.isTimerSet)
				{
					...
					textString = text.text = myGameplayController.translateTimer(((int)(angle * 360f)).ToString()).ToString();
