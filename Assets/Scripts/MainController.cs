...
public class MainController : MonoBehaviour
{
    ...
    public TimerManager myTimerManager;
    ...
    public TextAsset ConstellationSaveFile;
    ...
    public void LeaveChosenConstellation() 
    {
        myTimerManager.TurnOnTimer();
        MainScreen.transform.SetAsLastSibling();
    }
    ...
    {
        if (chosenConstellation != "None Chosen" || ConstellationSaveFile.text != "none") { GoToChosenConstellationScreen(); } else { 
    ...
    public void GoToChosenConstellationScreen()
    {
...

        {
            ...
            if (ConstellationSaveFile.text == "none")
            {
                chosenConstellation = EventSystem.current.currentSelectedGameObject.GetComponentInChildren<Text>().text;

                while (myChosenConstellationClass.name != ConstellationSaveFile.text)
                {
                    
                    if (myConstellationList.constellation[indexRaiser].name == chosenConstellation)
                    {
                        myChosenConstellationClass = myConstellationList.constellation[indexRaiser];
                        File.WriteAllText(Application.dataPath + "/Resources/JSONFiles/ChosenConstellationSave.txt", myChosenConstellationClass.name);
            ...
            else 
            {
                while (myChosenConstellationClass.name != ConstellationSaveFile.text)
                {
                    ...
                    if (myConstellationList.constellation[indexRaiser].name == ConstellationSaveFile.text)
                    {
                        myChosenConstellationClass = myConstellationList.constellation[indexRaiser];
                        File.WriteAllText(Application.dataPath + "/Resources/JSONFiles/ChosenConstellationSave.txt", myChosenConstellationClass.name);
                        AssetDatabase.Refresh();
            ...
            //   PRE SAVE FILE FUNCTIONALITY CODE 
            //        
